...
(function(){
    ...
    var io = new IntersectionObserver(function(entries){
      entries.forEach(function(en){
        ...
        if(en.isIntersecting){
          if(!v.src){
            /* adaptive resolution: big, capable desktops load the -hd file; weak/small screens keep the light one */
            var wantHD = (navigator.hardwareConcurrency||8) > 4 && (navigator.deviceMemory||8) > 4 && Math.min(window.screen.width, window.innerWidth) >= 1440;
            v.src = wantHD ? v.dataset.src.replace(/\.mp4$/, "-hd.mp4") : v.dataset.src;
          }
  ...
  if(hv){
    /* adaptive resolution: big, capable desktops load the HD file; weak/small screens keep the light file */
    (function(){
      var srcEl0 = hv.querySelector("source");
      var capable = (navigator.hardwareConcurrency||8) > 4 && (navigator.deviceMemory||8) > 4;
      var bigScreen = Math.min(window.screen.width, window.innerWidth) >= 1440;
      if(srcEl0 && capable && bigScreen){
        srcEl0.src = "hero-bg-hd.mp4";
        hv.load();
      }
    })();
    hv.preload = "metadata";
